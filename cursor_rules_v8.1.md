...
- **处理方式**：
  ...
  2. **Interactive Feedback**：精炼总结+询问后续需求
...
- **架构设计**："怎样设计..."、"最佳实践是..."
- **处理方式**：
  1. **立即调用 interactive_feedback**：询问用户需求和偏好
  2. **Cursor对话**：提供深度分析、技术对比、最佳实践
  3. **Interactive Feedback**：核心决策点+行动选项
...
- **问题修复**："修复这个bug..."、"解决错误..."
- **处理方式**：
  1. **立即调用 interactive_feedback**：确认操作范围和方式
  2. **Cursor对话**：提供完整实现方案和代码
  3. **执行具体操作**：使用相应工具完成任务
  4. **Interactive Feedback**：确认结果+询问后续需求
...
用户输入 → AI智能分类 + 性能优化策略
    ↓
├─ A级(智能直答)  → 快速响应模板 → 【强制】interactive_feedback询问后续
├─ B级(深度分析)  → 【强制】interactive_feedback → 标准分析模板 → 再次询问  
└─ C级(操作执行)  → 【强制】interactive_feedback → 完整实施方案 → 再次询问
    ↓
持续循环，直到用户明确结束指令
```
...
**记住：详细分析在Cursor，精炼总结在Feedback，图表说明必须有，文档生成需明确！**
...
- 🏷️ **设置类别**: bug|feature|review|performance|docs|test|deploy|other
